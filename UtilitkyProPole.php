...
require_once(dirname(__FILE__).'/UtilitkyProNazvy.php');

class UtilitkyProPole{
	...
	public static function dejKliceNaPozicich($pole, $pozice, $orezPrebytecnePole = true){
		...
		if($orezPrebytecnePole){
			return self::orezPole($klice);//pokud jsme nasli jen jeden klic, vratime ho samotny, bez pole
		}else{
			return $klice;
		}
	}
	...
	public static function dejPrvekNaPozici($pole, $pozice, $orezPrebytecnePole = true){
		...
		if($orezPrebytecnePole){
			return self::orezPole($polozky);//pokud jsme nasli jen jeden klic, vratime ho samotny, bez pole
		}else{
			return $polozky;
		}
	}
	...
	public static function orezPole($pole,$nahradPrazdnePolePrazdnymRetezcem = true){//rekurzivne zrusi pole, ktera obaluji jen jeden prvek a prazdna pole nahradi prazdnym stringem ""
		if(is_array($pole)){//menime jen pole
			foreach($pole as $index=>$prvek){//jdeme hloubeji
				$pole[$index] = self::orezPole($prvek);//rekurze - prohledani vsech vetvi pole az "na dren"
			}
	...
	public static function orezPoleNejnizsihoStupne($pole){//zrusi pole, ktere obsahuje uz jen jednu hodmotu a tou neni pole
		if(is_array($pole)){//menime jen pole
			if(sizeof($pole) === 1 and !is_array($pole[0])){//prvek v poli je stale array, ale obsahuje uz jen jeden prvek a ten neni pole
				...
				foreach($pole as $index=>$prvek){//jdeme hloubeji
					$pole[$index] = self::orezPoleNejnizsihoStupne($prvek);//rekurze - prohledani vsech vetvi pole az "na dren"
				}
			}
		}
		return $pole;
	...
	public static function orezPoleASmazPrazdno($pole){//rekurzivne smaze pole, ktera nic neobsahuji, zrusi pole, ktera obaluji jen jeden prvek a prazdna pole nahradi prazdnym stringem ""
		return self::smazPrazdneInformace(self::orezPole($pole));
	}
